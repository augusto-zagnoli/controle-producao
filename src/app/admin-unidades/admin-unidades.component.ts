...
@Component({
  ...
  styleUrl: './admin-unidades.component.scss',
})
export class AdminUnidadesComponent implements OnInit {
  ...
  abrirAdicionar(): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Unidade' } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
      this.service.criar({ tipo: this.tipo, nome: result.nome, ordem: result.ordem }).subscribe({
        next: () => { this.snack.open('Unidade adicionada.', 'OK', { duration: 3000 }); this.carregar(); },
  ...
  abrirEditar(item: OpcaoCampo): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Unidade', item } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
  ...
  excluir(item: OpcaoCampo): void {
    if (!confirm(`Excluir a unidade "${item.nome}"?`)) return;
