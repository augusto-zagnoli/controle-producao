...
@Component({
  ...
  templateUrl: './admin-funcoes.component.html',
  styleUrl: './admin-funcoes.component.scss',
})
export class AdminFuncoesComponent implements OnInit {
  ...
  abrirAdicionar(): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, {
      width: '420px',
      data: { tipo: 'Função' },
    });
    ref.afterClosed().subscribe(result => {
      ...
      this.service.criar({ tipo: this.tipo, nome: result.nome, ordem: result.ordem }).subscribe({
        next: () => { this.snack.open('Função adicionada.', 'OK', { duration: 3000 }); this.carregar(); },
  ...
  excluir(item: OpcaoCampo): void {
    if (!confirm(`Excluir a função "${item.nome}"?`)) return;
