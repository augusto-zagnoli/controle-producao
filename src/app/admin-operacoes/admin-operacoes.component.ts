...
@Component({
  ...
  templateUrl: './admin-operacoes.component.html',
  styleUrl: './admin-operacoes.component.scss',
})
export class AdminOperacoesComponent implements OnInit {
  ...
  abrirAdicionar(): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Operacao' } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
      this.service.criar({ tipo: this.tipo, nome: result.nome, ordem: result.ordem }).subscribe({
        next: () => { this.snack.open('Operação adicionada.', 'OK', { duration: 3000 }); this.carregar(); },
  ...
  abrirEditar(item: OpcaoCampo): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Operacao', item } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
  ...
  excluir(item: OpcaoCampo): void {
    if (!confirm(`Excluir a operação "${item.nome}"?`)) return;
    this.service.excluir(item.id).subscribe({
      next: () => { this.snack.open('Excluída.', 'OK', { duration: 3000 }); this.carregar(); },
