...
@Component({
  selector: 'app-admin-equipamentos',
  standalone: true,
  imports: [
    CommonModule, MatTableModule, MatButtonModule, MatIconModule,
    MatDialogModule, MatProgressBarModule, MatChipsModule, MatTooltipModule, MatSnackBarModule,
  ],
  templateUrl: './admin-equipamentos.component.html',
  styleUrl: './admin-equipamentos.component.scss',
})
export class AdminEquipamentosComponent implements OnInit {
  ...
  abrirAdicionar(): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Equipamento' } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
      this.service.criar({ tipo: this.tipo, nome: result.nome, ordem: result.ordem }).subscribe({
        next: () => { this.snack.open('Equipamento adicionado.', 'OK', { duration: 3000 }); this.carregar(); },
  ...
  abrirEditar(item: OpcaoCampo): void {
    const ref = this.dialog.open(OpcaoCampoDialogComponent, { width: '420px', data: { tipo: 'Equipamento', item } });
    ref.afterClosed().subscribe(result => {
      if (!result) return;
  ...
  excluir(item: OpcaoCampo): void {
    if (!confirm(`Excluir o equipamento "${item.nome}"?`)) return;
